function test_vnctools_connect_find_remote_vnc_session() {
    ...
    $(vnctools_connect::find_remote_vnc_session user host)
...
vnctools_connect::execute_remote_command user host vncserver -localhost
vnctools_connect::execute_remote_command user host vncserver -list | grep ^: | awk "{print \$1}"
EOF
)
     [[ ${actual} == ${expected} ]]
...
# function test_vnctools_connect_find_remote_vnc_session_none_available() {
#     local return_code=0
#     source $(which vnctools-connect)
#     $(vnctools_connect::find_remote_vnc_session user host "$(cat << EOF
# EOF
# )") || return_code=1
#     [[ $return_code -eq 1 ]]
# }

# function test_vnctools_connect_find_remote_vnc_session_one_available() {
#     source $(which vnctools-connect)
#     local actual=$(vnctools_connect::find_remote_vnc_session user host "$(cat << EOF
# :1
# EOF
# )")
#     local expected="1"
...
# function test_vnctools_connect_find_remote_vnc_session_two_available() {
#     source $(which vnctools-connect)
#     local actual=$(vnctools_connect::find_remote_vnc_session user host "$(cat << EOF
# :2
# :1
